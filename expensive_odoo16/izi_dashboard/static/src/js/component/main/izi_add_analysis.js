odoo.define('izi_dashboard.IZIAddAnalysis', function (require) {
    "use strict";
    ...
    var IZIAddAnalysis = IZIDialog.extend({
        jsLibs: [
            '/izi_dashboard/static/lib/ace/ace.js',
            [
                '/izi_dashboard/static/lib/ace/mode-sql.js',
                '/izi_dashboard/static/lib/ace/theme-chrome.js',
            ]
        ],
            ...
            if (self.selectedTable) {
                self._rpc({
                    ...
                    if (results.data && results.data.length > 0) {
                        swal('Success', `Your query is good to go! Here is the example results: ${results.message}`, 'success');
                    } else {
        ...
        _executeQuery: function (ev) {
            ...
            if (!self.selectedAnalysis) {
                swal('Failed', 'You have to save the analysis first before executing the query!', 'error');
            }
            if (self.selectedTable) {
                        ...
                        if (self.selectAnalysis) {
                            self._rpc({
                                ...
                                if (results.status == 200) {
                                    self._loadFields();
                                    swal('Success', `Your query is executed successfully. The fields has been reset.`, 'success');
                                } else {
                                    swal('Failed', results.message, 'error');
                                }
                            });
        ...
        _onSelectAnalysisItem: function (ev) {
            ...
            self.selectedDashboard = self.parent.selectedDashboard;
            if (self.selectedAnalysis && self.selectedDashboard) {
                self._rpc({
                    ...
                    self.parent._selectDashboard(self.parent.selectedDashboard, self.parent.selectedDashboardName, self.parent.selectedDashboardWriteDate, self.parent.selectedDashboardThemeName);
                    swal('Success', `Analysis has been added to ${self.parent.selectedDashboardName}!`, 'success');
                    // self.destroy();
                });
            }
        },
        ...
        _saveAnalysis: function() {
            ...
            if (!analysisName) {
                swal('Failed', 'Please input the analysis name!', 'error');
                return false;
            }
            if (!self.selectedSource) {
                swal('Failed', 'Please select the data source for this analysis!', 'error');
                return false;
            }
            if (!self.selectedTable) {
                swal('Failed', 'Please select the table for this analysis!', 'error');
                return false;
            }
            var data = {
                'name': analysisName,
                ...
                swal({
                    ...
                    if (yes) {
                        self._rpc({
                            ...
                            swal('Success', `Analysis has been deleted successfully`, 'success');
                            self._loadAnalysisItems();
                            self._openSlide(self.$analysisContainer);
                            self._closeSlide(self.$formContainer);
                        });
        ...
        _onClickSaveTable: function(ev) {
            ...
            if (!self.selectedSource) {
                swal('Failed', 'Please select the data source for this table!', 'error');
                return false;
            }
            if (!tableName) {
                swal('Failed', 'Please input the new table name!', 'error');
                return false;
            }
            if (tableName && self.selectedSource) {
                ...
                self._rpc({
                    ...
                    if (result) {
                        ...
                        swal('Success', `Table has been saved successfully. You can click anywhere to close the wizard.`, 'success');
                        self.$tableName.show();
                        self.$btnTableEdit.show();
                        self.$btnTableNew.show();
                        self.$btnTableSave.hide();
        ...
        _writeAnalysis: function (id, data) {
            ...
            self._rpc({
                ...
                swal('Success', `Analysis has been saved successfully. You can click anywhere to close the wizard.`, 'success');
