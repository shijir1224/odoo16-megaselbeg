class amChartsComponent {
    ...
    makeBarChart() {
        ...
        function createAxisAndSeries(field, i) {
            ...
            series.name = field.replaceAll('_', ' ').trim();
    ...
    makeBulletBarChart() {
        ...
        series.name =  (self.metric[0]).replaceAll('_', ' ').trim();
        ...
        if ((self.metric).length >= 2) {
            ...
            series2.name = (self.metric[1]).replaceAll('_', ' ').trim();
    ...
    makeRowLine() {
        ...
        function createLineSeries(field, i) {
            ...
            seriesLine.name = field.replaceAll('_', ' ').trim();
    ...
    makeScorecardBasic() {
        ...
        `;
        // Trends Growth 
        if (self.trends && (self.data).length >= 2) {
            ...
            if (growthNum > 0) {
                elmGrowthNum = ` <div class="scorecard-status label label-success">
                                    <span class="material-icons">arrow_drop_up</span>
                                    `+ growthNum +`% </div> `
            }else if(growthNum < 0){
                elmGrowthNum = ` <div class="scorecard-status label label-danger">
        ...
        if (recentAchievement > 0) {
            ...
            $("#" + self.idElm + " .scorecard-chart").append(
                `<svg class="circle-ring" viewBox="0 0 10 10">
                    ...
                    <text x="`+ cx +`" y="`+ cy +`" text-anchor="middle" dy="0.38em" class="percentage">` + recentAchievement.toLocaleString() + `%</text>
                </svg>`
            );
            self.addMicroChartCircleProgress(".circle-ring", recentAchievement, circleStrokeWidth, circleRadius, cx, cy);
        } else if (self.trends && (self.data).length >= 2) {
            self.addMicroChartLine(".scorecard-chart", "full-endpoint");
            $("#" + self.idElm + " .scorecard-status .label").attr({"data-toggle":"tooltip", "data-placement":"top", "title":"Since last period"});
        }
