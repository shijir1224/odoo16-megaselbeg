    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        for item in customer:
            if item:
                ...
                local_msg_vals['pid'] = item.id
                local_msg_vals['hash'] = self._sign_token(item.id)
                local_msg_vals.update(item.signup_get_auth_param()[item.id])
                access_link = self._notify_get_action_link('view', **local_msg_vals)

                new_group = [
                    ('portal_customer', lambda pdata: pdata['id'] == item.id, {
