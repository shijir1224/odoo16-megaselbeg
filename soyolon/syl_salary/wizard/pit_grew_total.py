...
class PittGrewNewReport(models.TransientModel):
    ...
    def export_report(self):
        ...
        query="""SELECT  he.name as name,
            ...
            sum((select coalesce(sum(amount),0) from salary_order_line_line ll left join hr_allounce_deduction_category cat ON cat.id=ll.category_id where ll.order_line_id1=line.id and cat.code='PITT')) as dis,
            ...
            WHERE so.company_id=%s and so.type='final' and so.month>='%s' and so.month<='%s'  and so.year='%s'
            GROUP BY he.name,he.last_name,he.passport_id,he.ttd_number,it.id"""%(self.company_id.id,self.s_month,self.e_month,self.year)
        ...
        for record in records:
            ...
            sheet.write(rowx, 6,'0',contest_right)
            sheet.write(rowx, 7,it_pool.shi_procent,contest_right)
            ...
            sheet.write(rowx, 13,'0',contest_right)
            ...
            sheet.write(rowx, 19,'0',contest_right)
