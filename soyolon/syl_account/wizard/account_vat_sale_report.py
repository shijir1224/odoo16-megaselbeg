...
class SylVatSaleReport(models.TransientModel):
	_name = 'syl.vat.sale.report'
	...
	def download_report(self):
		...
		file_name = 'Борлуулалтын дэвтэр.xlsx'
		...
		sheet.merge_range(0, 0, 0, 7, u'Борлуулалтын дэвтэр', header)
		...
		if self.partner_id:
			...
			domains_external_return.append(('partner_id','=',self.partner_id.id))
			domains_internal_return.append(('partner_id','=',self.partner_id.id))
			domains_undefined_return.append(('partner_id','=',self.partner_id.id))
		if self.journal_ids and self.include_account and self.account_ids:
			...
			domains_external_return.append(('move_id.journal_id','in',self.journal_ids.ids))
			domains_internal_return.append(('move_id.journal_id','in',self.journal_ids.ids))
			domains_undefined_return.append(('move_id.journal_id','in',self.journal_ids.ids))
			domains_external.append(('tax_ids','!=',False))
			domains_internal.append(('tax_ids','!=',False))
			domains_undefined.append(('tax_ids','!=',False))
			domains_external_return.append(('tax_ids','!=',False))
			domains_internal_return.append(('tax_ids','!=',False))
			domains_undefined_return.append(('tax_ids','!=',False))
			if self.include_account:
				...
				domains_external_return.append(('account_id','not in', self.account_ids.ids))
				domains_internal_return.append(('account_id','not in', self.account_ids.ids))
				domains_undefined_return.append(('account_id','not in', self.account_ids.ids))
			aml_in=self.env['account.move.line'].search(domains_internal, order='date asc')
			aml_ex=self.env['account.move.line'].search(domains_external, order='date asc')
			aml_un=self.env['account.move.line'].search(domains_undefined, order='date asc')
			aml_in_r=self.env['account.move.line'].search(domains_internal_return, order='date asc')
			aml_ex_r=self.env['account.move.line'].search(domains_external_return, order='date asc')
			aml_un_r=self.env['account.move.line'].search(domains_undefined_return, order='date asc')
			dd = 1
			...
			for move in aml_in:
				...
				if check == False and self.include_account:
					...
					sheet.write(row, 5, move.credit, contest_number_right)
					sheet.write(row, 6, '0', contest_number_right)
					sheet.write(row, 7, move.credit, contest_number_right)
					sheet.write(row, 8, ', '.join(move.tax_ids.mapped('name')) or '', contest_left)
					dd += 1
					row += 1
			for move in aml_in_r:
				check = any(ch in move.move_id.line_ids.account_id.ids for ch in self.account_ids.ids)
				if check == False and self.include_account:
					sheet.write(row, 0, dd, contest_left)
					sheet.write(row, 1, move.date.strftime("%Y-%m-%d"), contest_left)
					...
					sheet.write(row, 3, move.partner_id.name, contest_left)
					sheet.write(row, 4, move.partner_id.vat if move.partner_id.vat else '', contest_left)
					sheet.write(row, 5, move.debit*(-1), contest_number_right)
					sheet.write(row, 6, '0', contest_number_right)
					sheet.write(row, 7, move.debit*(-1), contest_number_right)
					sheet.write(row, 8, ', '.join(move.tax_ids.mapped('name')) or '', contest_left)
					dd += 1
					row += 1
			...
			domains_internal_return.append(('account_id','in',self.account_ids.ids))
			domains_external_return.append(('account_id','in',self.account_ids.ids))
			domains_undefined_return.append(('account_id','in',self.account_ids.ids))
			aml_in=self.env['account.move.line'].search(domains_internal, order='date asc')
			aml_ex=self.env['account.move.line'].search(domains_external, order='date asc')
			aml_un=self.env['account.move.line'].search(domains_undefined, order='date asc')
			aml_in_r=self.env['account.move.line'].search(domains_internal_return, order='date asc')
			...
			for move in aml_in:
				...
				sheet.write(row, 3, move.partner_id.name, contest_left)
				sheet.write(row, 4, move.partner_id.vat if move.partner_id.vat else '', contest_left)
				sheet.write(row, 5, sum(move.move_id.line_ids.mapped('credit')), contest_number_right)
				sheet.write(row, 6, move.credit, contest_number_right)
				sheet.write(row, 7, sum(move.move_id.line_ids.mapped('credit')) - move.credit, contest_number_right)
				dd += 1
				row += 1
			# for move in aml_in_r:
			# 	sheet.write(row, 0, dd, contest_left)
			...
			# 	sheet.write(row, 3, move.partner_id.name, contest_left)
			# 	sheet.write(row, 4, move.partner_id.vat if move.partner_id.vat else '', contest_left)
			# 	sheet.write(row, 5, sum(move.move_id.line_ids.mapped('debit'))*(-1), contest_number_right)
			# 	sheet.write(row, 6, move.debit*(-1), contest_number_right)
			# 	sheet.write(row, 7, (sum(move.move_id.line_ids.mapped('debit')) - move.debit)*(-1), contest_number_right)
			# 	dd += 1
			# 	row += 1
			sheet.merge_range(row, 0, row, 7, u'Б. Экспортын борлуулалт', contest_center)
			row += 1
			...
			# 	row += 1
			if aml_un or aml_ex or aml_in:
				sheet.write_formula(row, 5, '{=SUM('+self._symbol(7, 5) +':'+ self._symbol(row-1, 5)+')}', contest_number_right)
				sheet.write_formula(row, 6, '{=SUM('+self._symbol(7, 6) +':'+ self._symbol(row-1, 6)+')}', contest_number_right)
				sheet.write_formula(row, 7, '{=SUM('+self._symbol(7, 7) +':'+ self._symbol(row-1, 7)+')}', contest_number_right)
		else:
