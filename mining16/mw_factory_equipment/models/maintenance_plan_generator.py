...
class MaintenanceType(models.Model):
	...
	is_equipment = fields.Boolean(string="Тоног төхөөрөмжийн төрөл эсэх", default=False)

class EquipmentPlanGenerator(models.Model):
	...
	work_time_per_day = fields.Float(string=u'Өдөрт ажиллах цаг', states={'done': [('readonly', True)]})

	generate_type = fields.Selection([
			('only_one', u'Зөвхөн нэг'),
			('all', u'Бүх тоног төхөөрөмж'),
	...
	def action_to_confirm(self):
		...
		if self.generate_type == 'only_one':
			...
			if not self.technic_setting_line:
				if not self.env.context.get('equipment', False):
					raise UserError(_(u'Техникүүдийн мэдээллийг оруулна уу!'))
				# else:
				# 	raise UserError(_(u'Тоног төхөөрөмжүүдийн мэдээллийг оруулна уу!'))
	...
	def export_excel_template(self):
		...
		worksheet.write(0, 1, u"Техник" if not self.env.context.get('is_equipment', False) else 'Тоног төхөөрөмж', contest_left)
		...
		if self.technic_setting_line:
			...
			technics = self.env['technic.equipment'].search([('owner_type','=','own_asset'),
					('is_tbb_report','=',True)],
				order="report_order, program_code")
	...
	def generate_lines(self):
		...
		if self.generate_type == 'only_one':
			...
			if work_time_per_day == 0:
					raise UserError(_(u'11 Өдөрт ажиллах цаг 0 байна! %s %d' % (self.technic_id.technic_setting_id.name, technic.technic_setting_id.id)))
		...
		# Хэрэв олон техник үүсгэх гэж байгаа бол
			...
			for setting_line in self.technic_setting_line:
				...
				if work_time_per_day == 0:
					print(setting_line, technic, technic.park_number)
					print(technic.technic_setting_id)
					raise UserError(_(u'22 Өдөрт ажиллах цаг 0 байна! %s %d' % (technic.technic_setting_id.name, technic.technic_setting_id.id)))
				...
				while temp_date < self.date_end:
					...
					if pm_config.work_time <= 0:
						temp_date = self._date_increase(temp_date,days)
						print(pm_config, pm_config.work_time)
						continue
...
class EquipmentPlanGeneratorLine(models.Model):
	...
	equipment_id = fields.Many2one('factory.equipment', string='Тоног төхөөрөмж', required=True,)
	responent_id = fields.Many2one('res.partner', string=u'Хариуцагч')
	equipment_motoh = fields.Float(string="Ажилласан цаг")
	partner_id = fields.Many2one('res.partner', string="Ажил хариуцагч", domain="[('employee','=',True)]")

	
	@api.onchange('equipment_id')
	def onchange_equipment_motoh(self):
		for item in self:
			item.equipment_motoh = item.equipment_id.total_odometer

	
	def create_plan(self):
		for obj in self:
			# Сарынx бол төлөвлөгөө үүсгэхгүй
			if obj.parent_id.forecast_type not in ['weekly','monthly']:
				return
			# Material data
			material_datas = []
			...
			temp_work_time_2 = 0
			if obj.work_time <= 0:
				raise UserError(('Засварын цаг 0 их байх хэрэгтэй!'))
			if obj.work_time > shift_hour and obj.technic_id:
				temp_work_time = shift_hour
				temp_work_time_2 = obj.work_time - shift_hour

			# Өдрийн ПЛАН үүсгэх
			branch_id = obj.parent_id.branch_id
			vals = {
			...
			if obj.technic_id:
				vals.update({'technic_id': obj.technic_id.id})
			elif obj.equipment_id:
				vals.update({'equipment_id': obj.equipment_id.id})
...
class TechinicSeetingLine(models.Model):
	...
	def create_workorder(self):
		...
		if self.technic_id:
			vals.update({'technic_id': self.technic_id.id})
		if self.equipment_id:
			vals.update({'equipment_id': self.equipment_id.id})
