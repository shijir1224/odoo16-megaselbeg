...
class mining_cost_indirect_month(models.Model):
    _name = 'mining.cost.indirect.month'
    _description = 'Mining cost indirect month'
    _auto = False
    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE OR REPLACE VIEW mining_cost_indirect_month AS (
select 
...
where mcc.type='indirect_cost' and am.state='posted'
group by 1,2,3
order by 1
);
        """)
...
class mining_cost_config_indirect_amount(models.Model):
    _name = 'mining.cost.config.indirect.amount'
    _description = 'Mining cost config indirect amount'
    _auto = False
    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE OR REPLACE VIEW mining_cost_config_indirect_amount AS (
select 
...
left join mining_cost_indirect_month as by_month on 
