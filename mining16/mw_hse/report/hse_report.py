...
class hse_daily_report(models.TransientModel):
    ...
    branch_ids = fields.Many2many('res.branch', 'hse_daily_report_branch_rel', 'report_id','branch_id', 'Салбар')
    
    def get_injury(self, cr, uid, date, branch_ids, context=None):
        month = 0
        ytd = 0
        if date and branch_ids:
            date_object = datetime.strptime(date, '%Y-%m-%d')

            ytd = len(self.pool.get('hse.injury.entry').search(cr, uid, [('datetime','>=',str(date_object.year)+'-01-01'),('datetime','<=',date),('branch_id','in',branch_ids),('value','in',('FIRST_AID','MEDICAL_AID'))]))
            month = len(self.pool.get('hse.injury.entry').search(cr, uid, [('year','=',date_object.year),('month','=',date_object.month),('branch_id','in',branch_ids),('value','in',('FIRST_AID','MEDICAL_AID'))]))
            
        return {'month': month, 'ytd': ytd}
        
    def get_lost_time(self, cr, uid, date, branch_ids, context=None):
        ytd = 0
        month = 0
        if date and branch_ids:
            date_object = datetime.strptime(date, '%Y-%m-%d')
            ...
            cr.execute('SELECT sum(mmecl.diff_time) FROM mining_motohour_entry_cause_line mmecl ' 
                        ...
                        'WHERE mde.date>=%s AND mde.date<=%s AND mde.branch_id in %s AND mmecl.cause_id in (SELECT id FROM mining_motohours_cause WHERE is_injury=true)',
                           (year_start, date, tuple(branch_ids)))
            ...
            cr.execute('SELECT sum(mmecl.diff_time) FROM mining_motohour_entry_cause_line mmecl ' 
                        'LEFT JOIN mining_motohour_entry_line mmel ON (mmecl.motohour_cause_id = mmel.id) '
                        'LEFT JOIN mining_daily_entry mde ON (mmel.motohour_id = mde.id) '
                        'WHERE EXTRACT(YEAR FROM mde.date) = %s AND EXTRACT(MONTH FROM mde.date) = %s AND mde.date<=%s AND mde.branch_id in %s AND mmecl.cause_id in (SELECT id FROM mining_motohours_cause WHERE is_injury=true) ',
                           (date_object.year, date_object.month, date, tuple(branch_ids)))
    ...
    def get_nope_lti(self, cr, uid, date, branch_ids, context=None):
        ...
        if date and branch_ids:
            ...
            for item in range(1, 13):
                ...
                cr.execute('SELECT SUM(inc_man_hour::integer) as inc_man_hour FROM hse_nope_lti '
                            ...
                            'WHERE year=%s and month=%s and branch_id in %s) and branch_id in %s ',
                          (date_object.year, item, tuple(branch_ids), tuple(branch_ids)))
                ...
                lti = len(self.pool.get('hse.injury.entry').search(cr, uid, [('year','=',date_object.year),('month','=',item),('branch_id','in',branch_ids),('is_lti','=',True)]))
                ir = 0.0
                if man_hour != 0:
            ...
            cr.execute('SELECT SUM(inc_man_hour::integer), MAX(inc_total_day::integer) FROM hse_nope_lti WHERE date=%s and branch_id in %s',
                          (date, tuple(branch_ids)))
            inc_man_hour = 0
            inc_total_day = 0
            str_cr = cr.fetchone()
    ...
    def get_injury_detail(self, cr, uid, date, branch_ids, context=None):
        ...
        if date and branch_ids:
            ...
            for item in range(1, 13):
                ...
                for acc in accident_type_ids:
                    ...
                    inj_cnt = len(self.pool.get('hse.injury.entry').search(cr, uid, [('year','=',date_object.year),('month','=',item),('branch_id','in',branch_ids),('accident_type','=',acc)]))
                    data[acc] = inj_cnt
                    if inj_cnt==0:
                        data[acc] = None
                    # else:
    ...
    def get_all_hse(self, cr, uid, date, branch_ids, context=None):
        if date and branch_ids:
            date +=' 23:59:59'
            ...
            for item in branch_ids:
                ...
                'WHERE hsm.year>=%s AND hsm.month>=1 AND hsm.day>=1 AND hsm.date<=%s AND hsm.branch_id = %s GROUP BY hd.name',
                ...
                cr.execute('SELECT count(hwi.id),hd.name FROM hse_workplace_inspection hwi LEFT JOIN hr_department hd ON (hd.id = hwi.department_id) '
                "WHERE hwi.state IN ('sent_mail','repaired') AND hwi.year>=%s AND hwi.month>=1 AND hwi.day>=1 AND hwi.date<=%s AND hwi.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                'hhr.day>=1 AND hhr.datetime<=%s AND hhr.branch_id = %s'
                ...
                'hra.day>=1 AND hra.datetime<=%s AND hra.branch_id = %s'
                'GROUP BY hl.name ',(date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                "WHERE hie.year>=%s AND hie.accident_type in (select id from hse_accident_type where value='NEAR_MISS_INCIDENT') AND hie.month>=1 AND hie.day>=1 AND hie.datetime<=%s AND hie.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                cr.execute('SELECT count(hie.id),hd.name FROM hse_injury_entry hie LEFT JOIN hr_department hd ON (hd.id = hie.department_id) '
                "WHERE hie.year>=%s AND hie.accident_type in (select id from hse_accident_type where value='FIRST_AID') AND hie.month>=1 AND hie.day>=1 AND hie.datetime<=%s AND hie.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                cr.execute('SELECT count(hie.id),hd.name FROM hse_injury_entry hie LEFT JOIN hr_department hd ON (hd.id = hie.department_id) '
                "WHERE hie.year>=%s AND hie.accident_type in (select id from hse_accident_type where value='MEDICAL_AID') AND hie.month>=1 AND hie.day>=1 AND hie.datetime<=%s AND hie.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                cr.execute('SELECT count(hie.id),hd.name FROM hse_injury_entry hie LEFT JOIN hr_department hd ON (hd.id = hie.department_id) '
                "WHERE hie.year>=%s AND hie.accident_type in (select id from hse_accident_type where value='PROPERTY_DAMAGE') AND hie.month>=1 AND hie.day>=1 AND hie.datetime<=%s AND hie.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                cr.execute('SELECT count(hie.id),hd.name FROM hse_injury_entry hie LEFT JOIN hr_department hd ON (hd.id = hie.department_id) '
                "WHERE hie.year>=%s AND hie.accident_type in (select id from hse_accident_type where value='SPILL') AND hie.month>=1 AND hie.day>=1 AND hie.datetime<=%s AND hie.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                cr.execute('SELECT count(hie.id),hd.name FROM hse_injury_entry hie LEFT JOIN hr_department hd ON (hd.id = hie.department_id) '
                "WHERE hie.year>=%s AND hie.is_lti = 't' AND hie.month>=1 AND hie.day>=1 AND hie.datetime<=%s AND hie.branch_id = %s GROUP BY hd.name",
                (date_object.year, date_object, item))
                str_cr = cr.fetchall()
                data = {}
                sum_cnt = 0
                ...
                if len(send_ok)>0:
                    res.append({
                        'branch_id': item, 
                        'branch_name': self.pool.get('res.branch').browse(cr, uid, item).name, 
...
class hse_corrective_actions(models.TransientModel):
    ...
    name = fields.Char('Name', readonly=True, default='Corrective actions')
    ...
    branch_ids = fields.Many2many('res.branch', 'hse_corrective_actions_branch_rel', 'report_id','branch_id', 'Project', default=_get_branch_ids)
    
    def get_all(self, cr, uid,date_from, date_to, branch_ids, context=None):
        not_done = 0
        done = 0
        wi = []
        ...
        if branch_ids:
            cr.execute("select count(id) from hse_hazard_report where state in ('sent_mail') and branch_id in %s and datetime>=%s and datetime<=%s",
                              (tuple(branch_ids), start_date, end_date))
            str_cr = cr.fetchone()
            hazard_not_done = int(str_cr[0])

            cr.execute("select count(id) as count from hse_workplace_inspection where state in ('sent_mail') and branch_id in %s and date>=%s and date<=%s",
                              (tuple(branch_ids), start_date, end_date))
            str_cr = cr.fetchone()
            wi_not_done = int(str_cr[0])

            cr.execute("select count(id) as count from hse_injury_entry where state in ('closed','sent_mail') and branch_id in %s and datetime>=%s and datetime<=%s",
                              (tuple(branch_ids), start_date, end_date))
            str_cr = cr.fetchone()
            inj_not_done = int(str_cr[0])

            cr.execute("select count(id) as count from hse_hazard_report where state in ('repaired') and branch_id in %s and datetime>=%s and datetime<=%s ",
                              (tuple(branch_ids), start_date, end_date))
            str_cr = cr.fetchone()
            hazard_done = int(str_cr[0])

            cr.execute("select count(id) as count from hse_workplace_inspection where state in ('repaired') and branch_id in %s and date>=%s and date<=%s ",
                              (tuple(branch_ids), start_date, end_date))
            str_cr = cr.fetchone()
            wi_done = int(str_cr[0])

            cr.execute("select count(id) as count from hse_injury_entry where state in ('cor_act_closed') and branch_id in %s and datetime>=%s and datetime<=%s ",
                              (tuple(branch_ids), start_date, end_date))
            str_cr = cr.fetchone()
            ...
            for item in self.pool.get('hse.injury.entry').browse(cr, uid, self.pool.get('hse.injury.entry').search(cr, uid, [('state','=','sent_mail'),('branch_id','in',branch_ids),('datetime','>=',date_from),('datetime','<=',date_to)])):
                action_id = self.pool.get('ir.model.data')._xmlid_lookup(cr, uid, 'mw_hse', 'action_hse_injury_entry')[1]
                for line in item.corrective_action_line:
                    if not line.is_taken:
                        ...
                        data['branch']=item.branch_id.name
            ...
            for item in self.pool.get('hse.workplace.inspection').browse(cr, uid, self.pool.get('hse.workplace.inspection').search(cr, uid, [('state','=','sent_mail'),('branch_id','in',branch_ids),('date','>=',date_from),('date','<=',date_to)])):
                action_id = self.pool.get('ir.model.data')._xmlid_lookup(cr, uid, 'mw_hse', 'action_hse_workplace_inspection')[1]
                for line in item.wo_is_line:
                    if not line.is_repaired:
                        data={}
                        data['uzuulelt']=_('Workplace inspection')
                        ...
                        data['responsible'] = members_name
                        data['branch']=item.branch_id.name
                        data['atag']=u'<a href="'+unicode(base_url)+u'/web?db='+unicode(db_name)+u'#id='+str(item.id)+u'&view_type=form&model=hse.workplace.inspection&action='+str(action_id)+u'" target="_blank" '
                        res.append(data)
            
            
            for item in self.pool.get('hse.hazard.report').browse(cr, uid, self.pool.get('hse.hazard.report').search(cr, uid, [('state','=','sent_mail'),('branch_id','in',branch_ids),('datetime','>=',date_from),('datetime','<=',date_to)])):
                data={}
                ...
                data['branch']=item.branch_id.name
                action_id = self.pool.get('ir.model.data')._xmlid_lookup(cr, uid, 'mw_hse', 'action_hse_hazard_report')[1]
