...
class StockProductOtherExpenseInherit(models.Model):
    ...
    # Columns
    equipment_id = fields.Many2one('factory.equipment', u'Тоног төхөөрөмж', copy=False,
        states={'confirmed': [('readonly', True)], 'done': [('readonly', True)]})
    vin_number = fields.Char(related='equipment_id.vin_number', string = 'Тоног төхөөрөмжийн дугаар')
    ...
    def get_prepare_stock_move_line(self, line, sp_id, price_unit, desc, dest_loc):
        ...
        vals['equipment_id2'] = self.equipment_id.id or False
        print(vals)
        # print(aaa)
        return vals
...
class StockProductOtherExpenseLine(models.Model):
    ...
    equipment_id = fields.Many2one(related='parent_id.equipment_id')
    ...
    def _compute_analytic_mining_distribution(self):
        for line in self:
            ...
            if line.parent_id.equipment_id:
                config_id = self.env['product.account.config'].search([('equipment_ids','in',line.parent_id.equipment_id.id)], limit=1)
                line.account_id = config_id.account_id.id