	...
	def _compute_wc_messages(self):
		for item in self:
			...
			if item.id and product_ids:
				...
				sql_query = """SELECT prl.product_id,prl.equipment_id,pr.date,pr.employee_id,sum(prl.qty) as qty,pr.name
				...
				for qr in query_result:
					technic_name =''
					if qr['equipment_id']:
						technic_name = self.env['technic.equipment'].sudo().browse(qr['equipment_id']).display_name
			...
			if self.flow_id.is_technic == True:
				for item in self.line_ids:
					if not item.equipment_id:
						raise Warning((u'"%s" урсгал тохиргоо нь  дээр техник сонгох шаардлагатай. \n%s бараа нь дээр тоног төхөөрөмж сонго!'%(self.flow_id.name, item.product_id.display_name)))
...
class purchase_request_line(models.Model):
	...
	equipment_id = fields.Many2one('factory.equipment','Тоног төхөөрөмж')
...
class purchase_request_line_po_create(models.TransientModel):
	...
	def get_pr_po_line(self, product_id, re_lines, date, po_id, uom_id):
		with_technic = re_lines.filtered(lambda r: r.equipment_id)
		equipment_ids = with_technic.mapped('equipment_id')
		not_with_technic = re_lines.filtered(lambda r: not r.equipment_id)
		po_line_vals = []
		if not_with_technic:
			po_line_vals.append({
		...
		for item in equipment_ids:
			pr_ls = with_technic.filtered(lambda r: r.equipment_id==item)
			print ('----', pr_ls)
			po_line_vals.append({
				'product_id': product_id.id,
...
class purchase_order_line(models.Model):
	...
	def _create_stock_moves(self, picking):
		...
		for stock in res:
			if stock.purchase_line_id.equipment_id:
				stock.equipment_id2 = stock.purchase_line_id.equipment_id.id
	...
	def action_done(self):
		...
		for item in res:
			if item.equipment_id:
				item.po_line_ids.write({'equipment_id': item.equipment_id.id })
				# .technic_id =
				# item.purchase_order_line_id.technic_id = item.technic_id.id

				if item.internal_stock_move_id:
					item.internal_stock_move_id.equipment_id2 = item.equipment_id.id


class PrReport(models.Model):
	_inherit = "pr.report"

	equipment_id = fields.Many2one('factory.equipment','Тоног төхөөрөмж', readonly=True)
...
class PrReportExcel(models.TransientModel):
	...
	def get_tech_name(self, pr_line_id):
		...
		if pr_line_id and pr_line_id.equipment_id:
			return pr_line_id.equipment_id.park_number
		return super(PrReportExcel, self).get_tech_name(pr_line_id)

# Тениктэй холбоотой худалдан авалтын хүсэлтийг харуулах
class FactroyEquipment(models.Model):
	_inherit = 'factory.equipment'

	pr_request_line = fields.One2many('purchase.request.line', 'equipment_id',
		string='Худалдан авалтийн мөр', readonly=True,
		domain=['|',('po_line_ids.qty_received','=',0),('po_line_ids','=',False)]
		)

	pr_request_line_po = fields.One2many('purchase.request.line', 'equipment_id',
		string='Худалдан авалтийн мөр', readonly=True,
