...
class HseAllProInsReport(models.Model):

	_name = 'hse.all.pro.ins.report'
	_description = 'HSE Pro Ins Report'
	...
	def export_report(self):
		...
		sheet.write(rowx+1, 3, u'Компани/Company', theader),
		sheet.write(rowx+1, 4, u'Хэлтэс/Department', theader),
		...
		sheet.write(rowx+1, 13, u'Хүлээн авсан ажилтны гарын үсэг/Signature of  receiver', theader),
		rowx+=1
		...
		for record in records:	
			...
			hr_id = self.env['hr.employee'].search([('id','=',record['emp_id'])])
			...
			sheet.write(rowx, 3,hr_id.company_id.name,contest_left)
			sheet.write(rowx, 4,hr_id.department_id.name,contest_left)
			...
			sheet.write(rowx, 13,'',contest_center)
			rowx+=1
			n+=1
