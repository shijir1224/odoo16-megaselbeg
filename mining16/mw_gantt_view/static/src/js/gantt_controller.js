odoo.define('twb_gantt_view.GanttController', function (require) {
"use strict";
...
var GanttController = AbstractController.extend({
    events: _.extend({}, AbstractController.prototype.events, {
        ...
        'click .o_gantt_button_mw_srch': '_onMWSearchClicked',
    }),
...
//    },
    ...
    _onMWSearchClicked: function (ev) {
      ev.preventDefault();
      var state = this.model.get();
...
//      console.log(moment(date_val).format(format));
//      var focusDate = this.state.focusDate;
//      var dateStart = focusDate.clone().startOf('week').format('DD MMMM YYYY');
//      console.log('state.focusDate.subtract(1, state.scale) ',state.focusDate.subtract(1, state.scale));
      this.update({ date: moment(date_val) });
    },
