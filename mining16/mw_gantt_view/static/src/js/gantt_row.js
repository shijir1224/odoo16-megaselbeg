odoo.define('twb_gantt_view.GanttRow', function (require) {
    "use strict";
    ...
    var GanttRow = Widget.extend({
        ...
        init: function (parent, pillsInfo, viewInfo, options) {
            ...
            //            console.log('this.fieldsInfo=========== ', viewInfo.fieldsInfo);
        ...
        _aggregateGroupedPills: function () {
            ...
            this.pills.forEach(function (pill) {
                ...
                if (self.consolidate && self.consolidationParams.maxValue) {
                    ...
                    var ss = _.str.sprintf("background-color: rgba(%s, %s, %s, 0.6)", color, color, color);
                    //                console.log('_.str.sprintf("background-color:+++++++++++++++: ',ss);
                    pill.style = ss;
        ...
        _evaluateDecoration: function () {
            //console.log('_evaluateDecoration--------------- ');
            var self = this;
            var cnt = 0;
            this.pills.forEach(function (pill) {
                var pillDecorations = [];
                ...
                //            console.log('pillDecorations+++: ',pillDecorations);
                //            console.log('self.colorField+++: ',self.colorField);
                if (self.colorField) {
                    //            	console.log('pill+++: ',pill);
                    var cc = self._getColor(pill[self.colorField]);
                    //            	console.log('cc+++: ',cc);
                    pill._color = cc;
                }

                //console.log(pill.id);
                var model = 'car.repair.order'
                var fields = [];
                //                rpc.query({
                //                    model: model,
                //                    method: 'get_table_ro_info',
                //                    args: [
                //                        [fields], pill.id,
                //                    ]
                //                }).then(function (result) {
                //                    if (result) {
                ////                        console.log('chdjchdsjhcdjskchdjhsjd', result);
                ////                        console.log('car_modelcar_modelcar_model', result.car_model);
                ////                        console.log('this.pill--------------- ', pill);
                //                        pill.model_id = result.car_model;
                //                    }
                //                });
                //                console.log('asdasdasdasd', fields);
                //console.log('this.pill---------------pill.state ', pill.state);
                if (pill.display_name.includes('PHONE')) {
                    pill.tbw_font_color = '#FFFD33';
                } else {
                    pill.tbw_font_color = '#ffffff';
                }
                //                if (pill.display_name.includes('LXO')) {
                //                    pill.tbw_font_color = '#212529';
                //                }
                //console.log('this.pill.state ', pill);
                if (pill.state === 'draft') {
                    pill.tbw_state = '#454545';
                }
                if (pill.state === 'open') {
                    pill.tbw_state = '#454545';
                }
                if (pill.state === 'waiting_for_service') {
                    pill.tbw_state = '#F6AB61';
                }
                ...
                if (pill.state === 'waiting_for_inspection') {
                    pill.tbw_state = '#173f5f';
                }
                if (pill.state === 'waiting_for_delivery') {
                    pill.tbw_state = '#80D0C7';
                }
                if (pill.state === 'waiting_for_invoicing') {
                    pill.tbw_state = '#0199ff';
                }
                if (pill.state === 'call') {
                    pill.tbw_state = '#8C4D6C';
                }
                if (pill.state === 'psfu') {
                    pill.tbw_state = '#55C054';
                }
                if (pill.state === 'closed') {
                    pill.tbw_state = '#55C054';
                    cnt = cnt + 1;
                    //                    console.log('this.pill---------------pill.state ', cnt);
                }

            });
            self.cnt_close = cnt;
            //            console.log('cnt_close ' + self.cnt_close);
        },
        /**
        ...
        _getPopoverContext: function (pillID) {
            var data = _.clone(_.findWhere(this.pills, {
                id: pillID
            }));
        ...
        _saveResizeChanges: function (pillID, resizeDiff, direction) {
            var pill = _.findWhere(this.pills, {
                id: pillID
            });
            var data = {
                id: pillID
        ...
        _setResizable: function ($pill) {
            ...
            var pill = _.findWhere(self.pills, {
                id: $pill.data('id')
            });
        ...
        _onRowSidebarClicked: function () {
            if (this.isGroup & !this.isTotal) {
                if (this.isOpen) {
                    this.trigger_up('collapse_row', {
                        rowId: this.rowId
                    });
                } else {
                    this.trigger_up('expand_row', {
                        rowId: this.rowId
                    });
                }
            }
