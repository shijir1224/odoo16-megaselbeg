...
class report_mining_wizard(models.TransientModel):
    _name = "report.mining.wizard"  
    ...
    def open_analyze_mh(self):
        action = self.env.ref('mw_mining.action_mining_motohour_report_tree')
        vals = action.read()[0]
        vals['domain'] = self.get_domain()
        vals['context'] = {}
        return vals
    
    def open_analyze_prod(self):
        action = self.env.ref('mw_mining.action_mining_production_report_tree')
        vals = action.read()[0]
        vals['domain'] = self.get_domain()
        vals['context'] = {}
        return vals

    def open_analyze_ta(self):
        action = self.env.ref('mw_mining.action_mining_report_technic_analyze_tree')
        vals = action.read()[0]
        vals['domain'] = self.get_domain()
        vals['context'] = {}
        return vals
