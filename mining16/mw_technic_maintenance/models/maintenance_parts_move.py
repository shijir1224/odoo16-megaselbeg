...
class MaintenancePartsMove(models.Model):
	_name = 'maintenance.parts.move'
	_description = 'maintenance.parts.move'
	...
	from_technic_id = fields.Many2one('technic.equipment', string=u'Шилжүүлэн авах техник', copy=True, required=True,
		states={'confirmed':[('readonly', True)]})
	to_technic_id = fields.Many2one('technic.equipment', string=u'Шилжүүлж тавих техник', required=True,
		states={'confirmed':[('readonly', True)]})
	...
	line_ids = fields.One2many('maintenance.parts.move.line', 'parent_id', string='Lines', copy=False,
	...
	def action_planner(self):
		...
		if self.to_technic_id.id == self.from_technic_id.id:
			raise UserError(_(u'2 ижил техник сонгосон байна!'))
	...
	def action_to_create_pr(self):
		if self.is_create_pr:
			...
			search_domain = [('model_id.model','=','purchase.request'),'|',('branch_ids','=',[]),('branch_ids','in',[self.user_id.branch_id.id])]
			...
			if not flow_id:
				raise UserError('Худалдан авалтын хүсэлтийн урсгал тохиргоо олдсон')
			self.request_id = p_request.sudo().create(
				{'employee_id': emp.id if emp else False,
				'desc': self.description + self.from_technic_id.name + ' техник дээр Сэлбэг шилжилтээр үүсгэв.',
				'flow_id': flow_id.id,
			...
			for item in self.line_ids:
				vals = {
					...
					'technic_id': self.from_technic_id.id,
...
class MaintenancePartsMoveLine(models.Model):
	_name = 'maintenance.parts.move.line'
	_description = 'maintenance.parts.move.line'
	_order = 'product_id'
	# Columns
	parent_id = fields.Many2one('maintenance.parts.move', string=u'Parent', ondelete='cascade')
