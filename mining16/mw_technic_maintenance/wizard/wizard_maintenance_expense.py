...
class WizardMaintenanceExpense(models.TransientModel):
	_name = "wizard.maintenance.expense"
	...
	technic_id = fields.Many2one('technic.equipment', string='Technic',)
	...
	def open_plan_materials_report(self):
		if self.date_start and self.date_end:
			...
			search_res = mod_obj._xmlid_lookup(
				'mw_technic_maintenance.maintenance_expense_report_search')
			search_id = search_res and search_res[2] or False
			pivot_res = mod_obj._xmlid_lookup(
				'mw_technic_maintenance.maintenance_expense_report_pivot')
			pivot_id = pivot_res and pivot_res[2] or False
			tree_res = mod_obj._xmlid_lookup(
				'mw_technic_maintenance.maintenance_expense_report_tree_view')
	...
	def open_workorder_material_report(self):
		if self.date_start and self.date_end:
			...
			mod_obj = self.env['ir.model.data']
			search_res = mod_obj._xmlid_lookup(
				'mw_technic_maintenance.maintenance_wo_expense_report_search')
			search_id = search_res and search_res[2] or False
			pivot_res = mod_obj._xmlid_lookup(
				'mw_technic_maintenance.maintenance_wo_expense_report_pivot')
			tree_res = mod_obj._xmlid_lookup(
				'mw_technic_maintenance.maintenance_wo_expense_report_tree_view')
			pivot_id = pivot_res and pivot_res[2] or False
			...
			return {
				...
				'res_model': 'maintenance.wo.expense.report',
				'view_id': False,
				'views': [(tree_id, 'tree'),(pivot_id, 'pivot')],
				'search_view_id': search_id,
				'domain': domain,
	...
	def open_workorder_pivot_report(self):
		if self.date_start and self.date_end:
			...
			# GET views ID
			mod_obj = self.env['ir.model.data']
			search_id = self.env.ref('mw_technic_maintenance.maintenance_wo_report_search').id
			pivot_id = self.env.ref('mw_technic_maintenance.maintenance_wo_report_pivot').id
			tree_id = self.env.ref('mw_technic_maintenance.maintenance_wo_report_tree_view').id
			...
			return {
				...
				'view_type': 'form',
				'view_mode': 'pivot',
				'res_model': 'maintenance.wo.report',
				'view_id': False,
				'views': [(tree_id, 'tree'),(pivot_id, 'pivot')],
				'search_view_id': search_id,
				'domain': domain,
			...
			query="""
				...
				FROM (
					SELECT
						...
						ll.qty as plan_qty,
						0 as wo_qty
					FROM maintenance_expense_report as ll
				...
				GROUP BY tmp.report_order, tmp.technic_id, tmp.product_id, tmp.date
				ORDER BY tmp.report_order, tmp.technic_id, tmp.product_id, tmp.date
	...
	def get_done_workorder(self):
		if self.date_start <= self.date_end:
			...
			for line in wos:
				...
				worksheet.write(row, 3, line.technic_id.name or '', contest_left)
				worksheet.write(row, 4, line.technic_id.program_code, contest_center)
