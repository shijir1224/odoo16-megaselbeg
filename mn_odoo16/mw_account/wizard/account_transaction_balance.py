...
class AccountTransactionBalanceReport(models.TransientModel):
    ...
    chart_account_type = fields.Many2many('account.code.type', string="Дансны бүлэг")
    date_from = fields.Date(string='Start Date',default=time.strftime('%Y-%m-01'))
    ...
    @api.onchange('chart_account_type')
    def onchange_all_wh_import(self):
        if self.chart_account_type:
            self.chart_account_ids = self.env['account.account'].search([('code_group_id','=',self.chart_account_type.ids)])
        else:
            self.chart_account_ids = False
...
#         data['form'].update(self.read(['chart_account_ids'])[0])
        ...
        result.update(self.read(['chart_account_type'])[0])
...
#         data['form']['company_type'] = data['form']['company_type']
        ...
        datas = data
        domain = []
        action = self.env.ref('mw_account.action_account_transaction_balance_pivot_pivot')
        vals = action.read()[0]
