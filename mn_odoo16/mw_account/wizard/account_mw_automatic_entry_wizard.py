...
class AutomaticEntryWizard(models.TransientModel):
    _name = 'account.automatic.entry.mw'
    ...
    self_line_ids = fields.One2many('account.automatic.entry.mw.line', 'parent_id', string='Lines',default=_default_lines)
    ...
    def _onchange_self_line_ids(self):
        ...
        for l in self.self_line_ids:
            if l.tax_id:
                ...
                partner_id=l.filtered(lambda x:x.partner_id !=False).partner_id.id
    ...
    def _do_action_change_account(self, move_vals):
        ...
        for (partner, currency, account), lines in grouped_lines.items():
            if account.reconcile:
                ...
                to_reconcile.with_context(mw_auto=True).reconcile()
...
# select sum(amount_residual),sum(debit),sum(amount_residual_currency),sum(amount_currency) from (
# select amount_residual,reconciled,debit,credit,name,amount_residual_currency,amount_currency from account_move_line where id in (4510887, 4510791, 4510543, 4510432, 4510236, 4510095, 4509987, 4509853, 4509693, 4509509, 4509265, 4507927, 4507760, 4507432, 4506963, 4506296, 4506144, 4505555, 4324683, 4323192, 4632522, 4632523, 4632524, 4632525, 4632526, 4632527, 4632528, 4632529, 4632530, 4632531, 4632532, 4632533, 4632534, 4632535, 4632536, 4632537, 4632538, 4632539, 4632540, 4632541)
# )
#
# select id,amount_residual,reconciled,debit,credit,name,amount_residual_currency,amount_currency from account_move_line where id in (4510887, 4510791, 4510543, 4510432, 4510236, 4510095, 4509987, 4509853, 4509693, 4509509, 4509265, 4507927, 4507760, 4507432, 4506963, 4506296, 4506144, 4505555, 4324683, 4323192, 4632522, 4632523, 4632524, 4632525, 4632526, 4632527, 4632528, 4632529, 4632530, 4632531, 4632532, 4632533, 4632534, 4632535, 4632536, 4632537, 4632538, 4632539, 4632540, 4632541);
#
# update account_move_line set debit=foo.debit,   amount_residual=foo.amount_residual from (     
# select id,round(amount_residual,2) as amount_residual,
# reconciled,round(debit,2) as debit ,round(credit,2) as credit,name,amount_residual_currency,amount_currency from account_move_line where id in (4510887, 4510791, 4510543, 4510432, 4510236, 4510095, 4509987, 4509853, 4509693, 4509509, 4509265, 4507927, 4507760, 4507432, 4506963, 4506296, 4506144, 4505555, 4324683, 4323192, 4632522, 4632523, 4632524, 4632525, 4632526, 4632527, 4632528, 4632529, 4632530, 4632531, 4632532, 4632533, 4632534, 4632535, 4632536, 4632537, 4632538, 4632539, 4632540, 4632541)
# ) as foo where account_move_line.id=foo.id
...
class AutomaticEntryMW(models.TransientModel):
    _name = 'account.automatic.entry.mw.line'
    _description = 'Create Automatic Entries'
    ...
    parent_id = fields.Many2one('account.automatic.entry.mw', readonly=False, string="parent")
