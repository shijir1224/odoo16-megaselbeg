odoo.define("mw_pos_xac.payment", function (require) {
	"use strict";
	...
	var PaymentXac = PaymentInterface.extend({
		init: function () {
			this._super.apply(this, arguments);
		},

		get_xac_ok_payment: async function(cid){
			var self = this;
					...
					if (result.status === 200) {
						if (result.ecrResult.HostRespCode === '00') {
							...
							if(line){
								...
								line.xac_status = result.status;
								line.xac_textresponse = res_data.RespCode + ' '+ res_data.HostRespCode;
								line.xac_pan = res_data.PAN;
								line.xac_authorizationcode = res_data.ECRRefNo;
								line.xac_terminalid = res_data.TerminalID;
								line.xac_merchantid = res_data.MerchantName;
								line.xac_amount = res_data.TransAmount;
								line.xac_referencenumber = res_data.TraceNumber;
								
								console.log('result.ecrResult', result.ecrResult);
								line.xac_paid_ok = true;
								order.is_xac_ok_payment = true;
								if (!order.xac_paymentlines || order.xac_paymentlines==undefined){
									order.xac_paymentlines = [];
								}
								order.xac_paymentlines.push(line);
		...
		send_payment_request: function (cid) {
			...
			// 
			var self = this;
			var xac_ok = this.get_xac_ok_payment(cid);
			console.log('ugandaaaaa-++++++++++++++', xac_ok);
			if (xac_ok){
				var unpaid = this.get_xac_ok_unpaid_payment();
				if (unpaid){
					self.click_xac_sent(unpaid.unpaid_amount, unpaid.pline);
				}else{
					this._super();
		...
		_qpay_payment_pay: function () {
			...
			return this._qpay_payment_request(pay_line, pay_line.amount).then((result) => {
				console.log("+++++++++++++++++ xac result +++++++++++++++++++")
				console.log(result)
				if (result.confirmed == false) {
					pay_line.set_payment_status("waitingCard");
					return false;
				} else {
					console.log("XAC Payment success.................")
		...
		_show_error: function (msg, title) {
			...
			Gui.showPopup("ErrorPopup", {
				title: title || _t("Payment Xac Error"),
				body: msg,
			});
		},
