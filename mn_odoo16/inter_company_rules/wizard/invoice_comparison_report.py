...
class ComparisonReport(models.TransientModel):
    _name = "invoice.comparison.report"
    ...
    def export_report(self):
        ...
        worksheet.write(0,2, u"Харьцуулалтын тайлан /нэхэмжлэх/", h1)
        # TABLE HEADER
        row = 1
        ...
        worksheet.write(row, 2, u"Борлуулалт нэхэмжлэх", header_wrap)
        worksheet.set_column('C:C', 13)
        worksheet.write(row, 3, u"ХА нэхэмжлэх", header_wrap)
        worksheet.set_column('D:D', 13)
        worksheet.write(row, 4, u"ХА нэхэмжлэх дүн", header_wrap)
        worksheet.set_column('E:E', 12)
        worksheet.write(row, 5, u"Борлуулалт нэхэмжлэх дүн", header_wrap)
        worksheet.set_column('F:F', 12)
        worksheet.write(row, 6, u"Зөрүү", header_wrap)
        worksheet.set_column('G:G', 12)
        worksheet.write(row, 7, u"Зарлагын баримт", header_wrap)
        worksheet.set_column('H:H', 12)
        worksheet.write(row, 8, u"Зарлагын огноо", header_wrap)
        worksheet.set_column('I:I', 12)
        worksheet.write(row, 9, u"Орлогын баримт", header_wrap)
        worksheet.set_column('J:J', 12)
        worksheet.write(row, 10, u"Буцаалт нэх", header_wrap)
        worksheet.set_column('K:K', 12)
        worksheet.write(row, 11, u"Буцаалт дүн", header_wrap)
        worksheet.set_column('K:K', 12)
        worksheet.write(row, 12, u"Хүлээн авсан", header_wrap)
        ...
        query = """
            ...
            WHERE so.state in ('sale','done') and 
                  ...
                  so.is_create_auto_purchase = 't'
                   %s 
                   %s 
            GROUP BY sw.name, so.name, so.picking_date
            ORDER BY sw.name, so.picking_date, so.name
        ...
        for ll in qty_result:
            ...
            so_amount=0
            for po in pos:
                for am in po.invoice_ids:
                    if am.state=='posted':
                        po_inv=am.name
                        po_amount+=am.amount_total
            sos = self.env['sale.order'].sudo().search([
                ('name','=',ll['origin']),
                ('state','in',['sale','done'])])
            aguulax_str = ""
            for so in sos:
                for am in so.invoice_ids:
                    if am.state=='posted':
                        so_inv=am.name
                        so_amount+=am.amount_total
            ...
            for pr in ret_pos:
                for am in pr.invoice_ids:
                    if am.state=='posted':
                        pr_inv=am.name
                        return_amount+=am.amount_total
...
#                 print ('==return=', ll['origin'], pos.ids, ret_pos, return_qty)
            ...
            worksheet.write(row, 2, so_inv, contest_center)
            worksheet.write(row, 3, po_inv, contest_left)
            worksheet.write(row, 4, po_amount, contest_right)
            worksheet.write(row, 5, so_amount, contest_right)
            zurvv = so_amount - po_amount
            worksheet.write(row, 6, zurvv, contest_right if zurvv == 0 else contest_right_red)
            worksheet.write(row, 7, ll['origin'], contest_center)
            worksheet.write(row, 8, str(ll['picking_date']), contest_center)
            worksheet.write(row, 9, po.name or '-', contest_center)
            worksheet.write(row, 10, pr_inv or '-', contest_center)
            worksheet.write(row, 11, return_amount, contest_center)
            worksheet.write(row, 12, aguulax_str or '-', contest_left)
            row += 1
            number += 1
