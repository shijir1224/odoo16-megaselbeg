...
class ComplaintDocument(models.Model):
    ...
    type = fields.Selection(
        [('out', 'Гадаад'), ('in', 'Дотоод')], string='Төрөл')
    ...
    complaint_type = fields.Selection([('1', 'Ажлаас чөлөөлөгдөх тухай'), ('2', 'Ажилд орох тухай'), ('3', 'Тэтгэмж хүсэх тухай'), (
        '4', 'Урд хугацааны чөлөө хүсэх'), ('5', 'Хүүхэд асрах чөлөө хүсэх'), ('6', 'Хүүхэд асрах чөлөөнөөс эргэж орох')], 'Өргөдлийн төрөл')

    complaint_type_ids = fields.Many2one(
        'complaint.type', string='Өргөдлийн төрөл')

    complaint_employee_id = fields.Many2one(
        'hr.employee', string='Ажилтны нэр')
    complaint_job_id = fields.Many2one('hr.job', string='Албан тушаал')
    complaint_department_id = fields.Many2one(
    ...
    complaint_received = fields.Selection([('talk', 'Амаар'), ('phone', 'Утсаар'), (
        'write', 'Бичгээр')], 'Шийдвэрлэсэн өргөдлийн хариу илгээсэн байдал')
    complaint_idea = fields.Char(string='Өргөдлийн товч утга')
    complaint_date = fields.Date(
        string='Өргөдлийн огноо', default=fields.Date.context_today)
    ...
    name = fields.Char(string='Томилолтын зорилго')
...
# dynamic flow
    ...
    def _get_default_flow_id(self):
        ...
        search_domain.append(
            ('model_id.model', '=', 'complaint.document'))
    ...
    history_ids = fields.One2many(
        'dynamic.flow.history', 'complain_id', 'Түүхүүд')
    flow_id = fields.Many2one('dynamic.flow', string='Урсгалын тохиргоо', tracking=True, default=_get_default_flow_id,
                              copy=True, required=True, domain="[('model_id.model', '=', 'complaint.document')]")
        ...
        for item in self:
            if item.flow_id:
                item.visible_flow_line_ids = self.env['dynamic.flow.line'].search(
                    [('flow_id', '=', item.flow_id.id), ('flow_id.model_id.model', '=', 'complaint.document')])
            else:
    ...
    def flow_find(self, domain=[], order='sequence'):
        ...
        if self.flow_id:
            ...
            search_domain.append(
                ('flow_id.model_id.model', '=', 'complaint.document'))
        ...
        if next_flow_line_id:
            ...
            if next_flow_line_id._get_check_ok_flow(False, False):
                ...
                self.env['dynamic.flow.history'].create_history(
                    next_flow_line_id, 'complain_id', self)
    ...
    def action_back_stage(self):
        ...
        if back_flow_line_id:
            if back_flow_line_id._get_check_ok_flow(False, False):
                ...
                self.env['dynamic.flow.history'].create_history(
                    back_flow_line_id, 'complain_id', self)
            else:
    ...
    def send_chat_employee(self, partner_ids):
        ...
        html += u"""<b><a target="_blank" href=%s/web#id=%s&view_type=form&model=complaint.document&action=%s>%s</a></b>, томилолтын хүсэлт <b>%s</b> төлөвт орлоо""" % (
    ...
    def unlink(self):
        ...
        return super(ComplaintDocument, self).unlink()
...
class DynamicFlowHistory(models.Model):
    ...
    complain_id = fields.Many2one(
        'complaint.document', string='Хүсэлт', ondelete='cascade', index=True)
