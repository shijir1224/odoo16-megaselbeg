...
class ConsumableInventoryAdjustment(models.Model):
	_name = "consumable.inventory.adjustment"
	...
	line_ids = fields.One2many(
		comodel_name="consumable.inventory.adjustment.line", inverse_name="parent_id", copy=True
	)
	company_id = fields.Many2one(comodel_name="res.company", string="Company", 
		default=lambda self: self.env.company )
#     date_range_id = fields.Many2one(comodel_name="date.range", string="Date range")
	date = fields.Date(required=True, string="From Date",tracking=True)
#     date_to = fields.Date(required=True, string="To Date",tracking=True)
	state = fields.Selection([("draft", "Draft"), ("confirmed", "Confirmed"), 
							  ("cancelled", "Cancelled")], required=True, default="draft",)
	...
	category_id = fields.Many2one('consumable.material.category', 'Category')
	# location_id = fields.Many2one('account.asset.location', 'Location')
	all_asset = fields.Boolean('All Asset')
	import_data_ids = fields.Many2many('ir.attachment', 'consumable_inventory_adjustment_attach_import_data_rel', 'consum_inventory_id', 'attachment_id', 'Импортлох эксел', copy=False)

	def create_lines(self, interval=1):
		self.line_ids.unlink()    
		line_obj = self.env['consumable.inventory.adjustment.line']
		...
		for item in self:
			...
			if item.owner_dep_id:
				domains.append(('department_id','=',item.owner_dep_id.id))
				# if assets:
				#     for asset in assets:
				#         line=line_obj.create({
				#             'name': asset.name,
			...
			if item.all_asset==True:
				...
				for asset in assets:
					line=line_obj.create({
						...
						'description': asset.product_id.name,
						'asset_id': asset.id,
						'parent_id': False,
						'parent_id': item.id,
						'result': 'nocounted'
	...
	def action_open_inventory_lines(self):
		...
		action = {
			...
			'views': [(self.env.ref('mw_consume_order.view_consumable_inventory_adjustment_line_tree').id, 'tree')],
			'view_mode': 's',
			'name': _('Inventory Lines'),
			'res_model': 'consumable.inventory.adjustment.line',
		}
		context = {
...
#         worksheet.write(row, 0, row, last_col, item_cat.name, contest_left_bold)
		...
		worksheet.write(rowx, 2, u'Код', format_title)
...
#             for l  in line.line_ids:
			if l.asset_id:
				partner=''
				if l.asset_id.depreciation_line_ids:
					for i in l.asset_id.depreciation_line_ids:
						if i.move_id:
							partner=i.move_id.name
							break
				state=''
				if l.result=='counted':
					state='Тоологдсон'
				...
				value=l.asset_id.amount
				depreciated_amount = sum(l.asset_id.depreciation_line_ids.filtered(lambda r: r.move_id).mapped('amount'))
				worksheet.write(rowx, 0, n, format_group_left)
				worksheet.write(rowx, 1, l.asset_id.product_id.name, format_group_left)
				worksheet.write(rowx, 2, l.asset_id.product_id.default_code or '', format_group_left)
				worksheet.write(rowx, 3, l.asset_id.category_id and l.asset_id.category_id.name or '', format_group_left)
				worksheet.write(rowx, 4, get_date_format(l.asset_id.purchase_date), format_group_left)
#                 worksheet.write(rowx, 5,  l.asset_id.owner_emp_ids and l.asset_id.owner_emp_ids[0].name or '', format_group_left)
				worksheet.write(rowx, 5,  l.asset_id.owner_id and l.asset_id.owner_id.name +" "  + "/" or '', format_group_left)
				...
				worksheet.write(rowx, 11, l.asset_id.department_id and l.asset_id.department_id.name or '', format_group_left)
...
#         output = BytesIO()
		...
		file_name = "consumable_inventory_adjustment_%s.xlsx" % (time.strftime('%Y%m%d_%H%M'),)
		workbook.close()
	...
	def action_import_inventory(self):
		...
		for item in range(0,nrows):
			...
			if barcode:
				line=line_obj.search([('asset_id.product_id.default_code','=',barcode),('parent_id','=',self.id)])
				print ('line ',line)
				if len(line)>1:
					line=line_obj.search([('asset_id.product_id.default_code','=',barcode),
										  ('asset_id.product_id.name','=',name),
										  ('parent_id','=',self.id)])
					
				if line:
					line.write({'result':'counted'})
					   
						   
class AccountBudgetLine(models.Model):
	_name = "consumable.inventory.adjustment.line"
	_description = "Asset inventory line"
#     _rec_name= "conf_line_id"
	
	name = fields.Char("Name")
	code = fields.Char("Code", related='asset_id.product_id.default_code')
	
	description = fields.Char("description")

	asset_id = fields.Many2one('consumable.material.in.use','Asset')
	model_asset_id = fields.Many2one('consumable.material.category','Category',related='asset_id.category_id')
	branch_id = fields.Many2one('res.branch','Branch',related='asset_id.branch_id')
	owner_dep_id = fields.Many2one('hr.department', 'Owner department', related='asset_id.department_id')
	owner_emp_id = fields.Many2one('res.partner', 'Owner',related='asset_id.owner_id')
	date = fields.Date(string="Date",related='asset_id.purchase_date')
...
#     repair_hour = fields.Float(compute='_compute_run_hour', string='Repair hour', store = True, tracking=True)
	...
	parent_id = fields.Many2one("consumable.inventory.adjustment","Budget",ondelete="cascade",)

#     period_line_ids = fields.One2many(
#         comodel_name="consumable.inventory.adjustment.period.line", inverse_name="parent_line_id", copy=True
#     )
...
#     original_value = fields.Float(string="Анхны өртөг",related='asset_id.original_value',store=True)
	...
	result = fields.Selection(
		[("counted", "Тоологдсон"), ("nocounted", "Тоологдоогүй"), ("manual", "Гараар оруулсан")],
		required=True,
		default="nocounted",
		tracking=True,
	)
	...
	@api.depends(
				...
				'asset_id.depreciation_line_ids.move_id',
				'asset_id.depreciation_line_ids.amount')
	def _compute_total_amount(self):
		for item in self:
			depreciation_value=0
			original_value=0
			if item.asset_id:
				original_value=item.asset_id.amount
				for line in item.asset_id.depreciation_line_ids:
					if line.move_id:
						depreciation_value+=line.amount
			item.depreciation_value = depreciation_value
			item.original_value=original_value
			item.balance=original_value-depreciation_value
