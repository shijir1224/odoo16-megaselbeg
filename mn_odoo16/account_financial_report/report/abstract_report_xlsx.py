...
class AbstractReportXslx(models.AbstractModel):
        ...
        for col_pos, column in report_data["columns"].items():
            if column.get("field_initial_balance"):
                ...
                if cell_type == "string":
                    ...
                    # if my_object["currency_id"]: #ДАНС ДЭЭР ВАЛЮТГҮЙ БАЙСАН Ч ЭХНИЙ ҮЛДЭГДЭЛД ГАРГАХ
                        format_amt = self._get_currency_amt_format(
                            my_object, report_data
                        )
                        report_data["sheet"].write_number(
    ...
    def write_ending_balance_from_dict(self, my_object, name, label, report_data):
        ...
        for col_pos, column in report_data["columns"].items():
            ...
            if column.get("field_currency_balance"):
                value = my_object.get(column["field_currency_balance"], False)
                cell_type = column.get("type", "string")
                if cell_type == "many2one":
                    if my_object["currency_id"]:
                ...
                if my_object.get('amount_residual_currency',False):
                    format_amt_bold = self._get_currency_amt_bold_format_dict(
                        my_object, report_data
                    )
                    report_data["sheet"].write_number(
                        report_data["row_pos"], 10, float(my_object["amount_residual_currency"]), format_amt_bold #report_data["formats"]["format_header_amount"],
                    )                    
                    report_data["sheet"].write_number(
                        report_data["row_pos"], 11, float(my_object["amount_residual_currency"]), 
                        format_amt_bold #report_data["formats"]["format_header_amount"],
                    )                    
                    # report_data["sheet"].write_number(
                    #     report_data["row_pos"],
                    #     10,
                    #    float(my_object["amount_residual_currency"]),
                    #     report_data["formats"]["format_header_right"],
                    # )
    ...
    def _get_currency_amt_bold_format_dict(self, line_dict, report_data):
        """Return amount format specific for each currency."""
        format_amt = report_data["formats"]["format_amount_bold"]
        format_amt = report_data["workbook"].add_format({"bold": True, "border": True, "bg_color": "#FFFFCC"})
        ...
        if line_dict.get("currency_id", False) and line_dict["currency_id"]:
            ...
            currency_id = self.env["res.company"]._default_currency_id()
            report_data["field_name"] = format_amt
            format_amt.set_num_format(self._report_xlsx_currency_format(currency_id))
        return format_amt
